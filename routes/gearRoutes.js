...
module.exports = function(app) {
    ...
    // GET route for filtering for packing list items (entire row), in other words if the itemQuantityInPackingList boolean value is true ---??????????????????
    ...
    // ADD route for adding new item (row) to Gear


    // DELETE route for deleting items (entire row) from Gear


    // EDIT route for editing item values in the Gear ------------ Do we need separate routes for each data value we want to edit????????????????
}